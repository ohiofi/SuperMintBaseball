class Controller {
    ...
    update() {
        this.gameMessages = this.model.world.nextGameMessages();
        for (let i = 0; i < gameMessages.length; i++) {
            // update home page scores
            this.view.homePage.listGroupScores[i].innerHTML = gameMessages[i].getScoreString();

            // update game widgets
            this.view.liveGamesPage.widgets[i].update(gameMessages[i]);

            // update headlines on single game pages
            this.view.singleGamePages[i].headlineGameScore.innerHTML = gameMessages[i].getScoreString();
            // update box scores
